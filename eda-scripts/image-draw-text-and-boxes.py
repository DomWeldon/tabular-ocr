...
lineColor                           =   (0,0,255)
fontColor                           =   (255,0,0)
thresholder                         =   lambda x: 0 if x < 128 else 255
...
for filename in os.listdir(inputDir):
    ...
    wordBoxes                       =   tool.image_to_string(
        Image.open(os.path.join(inputDir, filename)).convert('L').point(thresholder, '1'),
        lang=lang,
    ...
    for box in wordBoxes:
        ...
        cv2.putText(img, box.content.encode('utf-8'), box.position[0], cv2.FONT_HERSHEY_SIMPLEX, 1, fontColor, 2)
    print tab(3), done()
